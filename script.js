...
const drugNamesLowerCase = drugs.map((drug) => {
...
const drugsInCategory = (drugCategory) => {
  return drugs.filter((drug) => {
    return drug.category === drugCategory;
  });
};
console.log(drugsInCategory("Supplement"));
...
noPrescriptionDrugs = drugs.filter((drug) => {
...
const drugManufacturer = (manufacturerName) => {
    return drugs.filter((drug) => {
        return drug.manufacturer === manufacturerName;
    }).length;
}
console.log(drugManufacturer("Teva"));



// 10. Use forEach() to count how many drugs are Analgesics
let numberOfAnalgesic = 0;
...
console.log(`Number of Analgesics: ${numberOfAnalgesic}`);
